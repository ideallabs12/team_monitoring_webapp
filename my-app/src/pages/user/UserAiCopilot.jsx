...
export default function UserAiCopilot({ user }) {
  ...
  useEffect(() => {
    async function fetchUserData() {
      ...
      try {
        ...
        const [profRes, revRes, disRes, targetRes] = await Promise.all([
          supabase.from('profiles').select('*, teams(id, name)').eq('id', user.id).single(),
          supabase.from('monthly_revenues').select('*').eq('user_id', user.id).order('revenue_month', { ascending: false }),
          supabase.from('dis_reports').select('*').eq('user_id', user.id).order('report_date', { ascending: false }).limit(30),
          supabase.from('revenue_targets').select('*').eq('user_id', user.id).order('target_month', { ascending: false })
        ])
        ...
        setMessages([
          {
            id: 'welcome',
            role: 'assistant',
            content: `👋 **Welcome, ${firstName}!** I am your **AI Copilot**.\n\nI'm directly connected to your live revenue logs, team assignments, and DIS reports. How can I help power your performance today? Choose a prompt below or ask me anything!`
          }
        ])
      } catch (err) {
        console.error('Failed to load user context for AI Copilot:', err)
        setErrorMsg('Could not load your performance records. Some AI insights might be general.')
      } finally {
  ...
  const stats = useMemo(() => {
    ...
    // Active target for current month
    const activeTarget = userTargets.find(t => normalizeMonth(t.target_month) === currentMonthStr)?.target_amount || null
  ...
  }, [userRevenues, userDisReports, userTargets])
  ...
  const quickPrompts = [
    {
      ...
      icon: Target,
      prompt: 'How close am I to my revenue target this month, and what daily run-rate or weekly deals do I need to hit it?'
    },
    {
      title: 'Revenue Trends',
      icon: TrendingUp,
      prompt: 'Analyze my revenue trends over recent months. Highlight growth patterns, dips, and average deal size.'
    },
    {
      title: 'Best Source Analysis',
      icon: Compass,
      prompt: 'Which deal source (Instagram, LinkedIn, etc.) generates the highest revenue for me, and how should I prioritize my outreach?'
    },
    {
      title: 'Weekly Action Plan',
      icon: Zap,
      prompt: 'Give me 3 high-impact, actionable sales strategies to boost my closed revenue for the current week.'
    },
    {
      title: 'Lead Pipeline (DIS)',
      icon: FileText,
      prompt: 'Based on my recent DIS reports and positive leads, what is my pipeline health and expected conversion rate?'
    }
  ]
...
You are the personal AI Sales Copilot and Performance Coach for "${userProfile?.first_name || 'Team Member'} ${userProfile?.last_name || ''}".
Company/Platform: Ideallabs (B2B SaaS & Growth Operations).
Current Date: ${now.toDateString()} (Month: ${currentMonthName} ${currentYear}).

USER LIVE PERFORMANCE CONTEXT:
- Team: ${userTeam?.name || 'Assigned Team'}
- Role: ${userProfile?.platform_role || 'Sales Executive'}
- This Month's Logged Revenue (${currentMonthName} ${currentYear}): $${stats.thisMonthTotal.toFixed(2)}
- Current Month Revenue Target: ${stats.activeTarget ? `$${Number(stats.activeTarget).toFixed(2)}` : 'No explicit target set yet'}
...
- Revenue by Source Breakdown: ${JSON.stringify(stats.sourceBreakdown)}
- Monthly Revenue History: ${JSON.stringify(stats.monthlySummary)}
- Recent DIS Activity (last 30 reports): ${stats.totalLeads} positive leads, ~$${stats.totalExpected.toFixed(2)} expected revenue

COACHING & RESPONSE SIZING RULES:
1. Speak directly to "${userProfile?.first_name || 'Team Member'}" in an encouraging, highly analytical, and strategic tone.
2. Ground all calculations in their REAL numbers provided above. Never invent fake revenue or contradict their live records.
...
`
  ...
  const handleClearChat = () => {
    const firstName = userProfile?.first_name || 'Team Member'
    setMessages([
      {
        id: 'welcome',
        role: 'assistant',
        content: `👋 **Fresh start, ${firstName}!** How can I assist with your revenue strategy or pipeline today?`
      }
    ])
    setErrorMsg('')
  }
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <div className="apple-kicker" style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--apple-accent-blue)' }}>
            <Sparkles size={14} /> Personal Revenue Intelligence
          </div>
          ...
          <p className="apple-lead" style={{ margin: 0 }}>
            Your personal AI sales strategist and performance coach, powered by your live metrics.
          </p>
        </div>
      ...
      <div style={{
        ...
        <div className="apple-card" style={{ padding: '14px 18px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--apple-border)' }}>
          ...
          <div style={{ fontSize: '1.05rem', fontWeight: '700', color: '#fff' }}>
            {userTeam?.name || 'Direct / Independent'}
          </div>
        </div>
        ...
        <div className="apple-card" style={{ padding: '14px 18px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--apple-border)' }}>
          ...
          <div style={{ fontSize: '1.2rem', fontWeight: '700', color: stats.activeTarget ? 'var(--apple-accent-blue)' : 'var(--apple-text-secondary)' }}>
            {stats.activeTarget ? `$${Number(stats.activeTarget).toLocaleString()}` : 'Not Set'}
          </div>
        </div>
