    ...
    <div>
      <div className="apple-card" style={{ padding: '24px', marginBottom: '24px', borderRadius: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '20px', marginBottom: '24px' }}>
          <div>
            <h2 style={{ margin: '0 0 8px 0', fontSize: '1.6rem', fontWeight: '800', color: 'var(--apple-text-primary)' }}>
              {mode === 'brackets' ? 'Revenue Ranges' : 'Zero Revenue Streaks'}
            </h2>
            <p style={{ margin: 0, color: 'var(--apple-text-secondary)', fontSize: '0.9rem' }}>
              {mode === 'brackets' ? 'Filter by revenue range for a given month.' : 'Users with zero revenue across consecutive months.'}{' '}
              <span style={{ color: 'var(--apple-accent-blue)', fontWeight: '600' }}>({displayList.length} found)</span>
            </p>
          </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem', textAlign: 'left' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--apple-border)', color: 'var(--apple-text-secondary)' }}>
                <th style={{ padding: '12px 8px' }}>Name</th>
                <th style={{ padding: '12px 8px' }}>Team</th>
                {mode === 'brackets' && <th style={{ padding: '12px 8px', textAlign: 'right' }}>Revenue</th>}
              </tr>
            </thead>
            <tbody>
              {displayList.map(u => (
                <tr key={u.id} style={{ borderBottom: '1px solid var(--apple-border)', color: 'var(--apple-text-primary)' }}>
                  ...
                  <td style={{ padding: '12px 8px', color: 'var(--apple-text-secondary)' }}>{u.teamName}</td>
                  {mode === 'brackets' && <td style={{ padding: '12px 8px', textAlign: 'right', fontWeight: '600', color: '#34d399' }}>${u.totalRev.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>}
                </tr>
              ))}
              {displayList.length === 0 && (
                <tr><td colSpan={mode === 'brackets' ? 3 : 2} style={{ textAlign: 'center', padding: '40px', color: 'var(--apple-text-secondary)' }}>No users found for this analysis.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
with open('src/components/stats/RevenueAnalysisModule.jsx', 'w', encoding='utf-8') as f:
    f.write(rev)
print('RevenueAnalysisModule.jsx written:', len(rev), 'chars')
