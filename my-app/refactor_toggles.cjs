...
files.forEach(f => {
  ...
  let modified = content.replace(/style=\{\{\s*width:\s*'44px',\s*height:\s*'24px',\s*borderRadius:\s*'12px',\s*(?:background|backgroundColor):\s*([^?]+)\s*\?\s*'#4ade80'\s*:\s*'#475569',\s*border:\s*'none',\s*position:\s*'relative',\s*cursor:\s*'pointer',\s*transition:\s*'background 0\.3s'\s*\}\}/g, 
  (match, p1) => {
